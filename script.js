...
// WEB v6.22 — Simplify officer portrait lightbox.
// Reuse the existing Command lightbox, but officer portraits open as image-only:
// no caption and no previous/next navigation. The group photo keeps its existing behavior.
(() => {
  const officerButtons = [...document.querySelectorAll('.command-carousel .command-image-button[data-command-index]')];
  const lightbox = document.querySelector('.command-lightbox');
  if (!officerButtons.length || !lightbox) return;

  const prev = lightbox.querySelector('.command-lightbox-prev');
  const next = lightbox.querySelector('.command-lightbox-next');
  const caption = lightbox.querySelector('figcaption');

  const setOfficerMode = isOfficer => {
    lightbox.classList.toggle('command-lightbox-officer-only', isOfficer);
    if (prev) prev.hidden = isOfficer;
    if (next) next.hidden = isOfficer;
    if (caption) caption.hidden = isOfficer;
  };
  ...
  // Main group image restores the standard lightbox presentation.
  const groupButton = document.querySelector('.command-showcase-image[data-command-index="0"]');
  if (groupButton) {
    groupButton.addEventListener('click', () => setOfficerMode(false), { capture:true });
  }

  // Reset after close so the next opening starts cleanly.
  const close = lightbox.querySelector('.command-lightbox-close');
  if (close) close.addEventListener('click', () => setOfficerMode(false));
  lightbox.addEventListener('click', event => {
    if (event.target === lightbox) setOfficerMode(false);
  });
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape') setOfficerMode(false);
  });
})();
