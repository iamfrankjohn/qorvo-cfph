...
function buildCfphArticleUrl(post) {
  // Prefer the exact working URL returned by the self-hosted checker.
  // STOVE can label a post as Event while still serving it under /News/Detail/.
  if (post?.url && /^https:\/\/cfph\.onstove\.com\//i.test(post.url)) {
    return post.url;
  }

  const id = Number(post?.id);
  if (Number.isInteger(id) && id > 0) {
    return `https://cfph.onstove.com/News/Detail/${id}?category=1&searchText=`;
  }

  return 'https://cfph.onstove.com/News/';
}
...
async function loadServerIntel() {
  ...
  try {
    const response = await fetch('https://cfph.koufuprinting.com/news', { cache: 'no-store' });
    ...
    if (postsBox) {
      ...
      posts.forEach((post, index) => {
        ...
        summary.textContent = cleanText(post.summary || post.description, 125) || 'Open the official CrossFire Philippines page for full details.';
    ...
    setMedia('server-intel-media', null, 'https://media.koufuprinting.com/https://media.koufuprinting.com/assets/stove-brand.svg');
  } catch (error) {
...
async function loadLatestFacebookPost() {
  ...
  const fallbackImage = new URL('https://media.koufuprinting.com/https://media.koufuprinting.com/assets/qorvo-cover.jpg', window.location.href).href;
...
async function loadQorvoReels() {
  ...
  function openPlayer(reel, index) {
    ...
    modalVideo.poster = reel.thumbnail || 'https://media.koufuprinting.com/https://media.koufuprinting.com/assets/qorvo-cover.jpg';
  ...
  function render() {
    ...
    reels.forEach((reel, index) => {
      ...
      const img = document.createElement('img');
      img.src = reel.thumbnail || 'https://media.koufuprinting.com/https://media.koufuprinting.com/assets/qorvo-cover.jpg';
      img.alt = `QORVO CFPH ${reelLabel(index)} thumbnail`;
      img.loading = index === active ? 'eager' : 'lazy';
      img.addEventListener('error', () => { img.src = 'https://media.koufuprinting.com/https://media.koufuprinting.com/assets/qorvo-cover.jpg'; }, { once: true });
  ...
  try {
    ...
    stage.innerHTML = `
      <a class="reels-fallback" href="${reelsPage}" target="_blank" rel="noopener">
        <img src="https://media.koufuprinting.com/https://media.koufuprinting.com/assets/qorvo-cover.jpg" alt="QORVO CFPH">
...
function addFaqMessage(text, sender) {
  ...
  if (sender === 'bot') {
    ...
    avatar.src = 'https://media.koufuprinting.com/assets/qorvo-mark.png';
    avatar.alt = '';
...
(()=>{const items=[['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/qorvo-command-group.webp','QORVO CFPH Clan Master and Officers group portrait','QORVO CFPH // CLAN MASTER & OFFICERS'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/iox-q-clan-master.webp','IOX.Q — QORVO Clan Master','IOX.Q // CLAN MASTER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/t3r-q-officer.webp','T3r.Q — QORVO Officer','T3r.Q // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/daichi-q-officer.webp','Daichi.Q — QORVO Officer','DAICHI.Q // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/haeqt-officer.webp','HaeQt. — QORVO Officer','HAEQT. // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/reed-q-officer.webp','Reed.Q — QORVO Officer','REED.Q // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/jvra-q-officer.webp','JVRA.Q — QORVO Officer','JVRA.Q // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/neonqt-officer.webp','NeonQt. — QORVO Officer','NEONQT. // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/pynouc-q-officer.webp','Pynouc.Q — QORVO Officer','PYNOUC.Q // OFFICER'],['https://media.koufuprinting.com/https://media.koufuprinting.com/assets/command/grim-q-officer.webp','Grim.Q — QORVO Officer','GRIM.Q // OFFICER']];const m=document.getElementById('command-lightbox'),im=document.getElementById('command-lightbox-image'),cap=document.getElementById('command-lightbox-caption'),cl=document.getElementById('command-lightbox-close'),pr=document.getElementById('command-lightbox-prev'),nx=document.getElementById('command-lightbox-next');if(!m||!im)return;let a=0;const r=()=>{im.src=items[a][0];im.alt=items[a][1];cap.textContent=items[a][2]},o=i=>{a=Number(i)||0;r();m.classList.add('is-open');m.setAttribute('aria-hidden','false');document.body.classList.add('command-lightbox-open')},s=()=>{m.classList.remove('is-open');m.setAttribute('aria-hidden','true');document.body.classList.remove('command-lightbox-open')},mv=d=>{a=(a+d+items.length)%items.length;r()};document.querySelectorAll('[data-command-index]').forEach(b=>b.addEventListener('click',()=>o(b.dataset.commandIndex)));cl.addEventListener('click',s);pr.addEventListener('click',()=>mv(-1));nx.addEventListener('click',()=>mv(1));m.addEventListener('click',e=>{if(e.target===m)s()});document.addEventListener('keydown',e=>{if(!m.classList.contains('is-open'))return;if(e.key==='Escape')s();if(e.key==='ArrowLeft')mv(-1);if(e.key==='ArrowRight')mv(1)})})();
